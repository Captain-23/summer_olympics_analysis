...
def fetch_medal_tally(df, years, country):
    # remove duplicates for fair medal counting
    ...
    if years == 'Overall' and country == 'Overall':
        ...
        temp_df = medal_df[
            (medal_df['Year'] == int(years)) & (medal_df['region'] == country)
        ]
    ...
    # Add total column
    tally_df['Total'] = tally_df['Gold'] + tally_df['Silver'] + tally_df['Bronze']

    # Ensure integer type
    tally_df[['Gold', 'Silver', 'Bronze', 'Total']] = tally_df[
...
def most_successful(df, sport):
    ...
    return x.merge(df, on='Name', how='left')[['Name', 'Count', 'region', 'Sport']].drop_duplicates('Name')
...
def most_successful_countrywise(df, country):
    ...
    return x.merge(df, on='Name', how='left')[['Name', 'Count','Sport']].drop_duplicates('Name')
...
def men_vs_women(df):
    ...
    men_vs_women = men.merge(women, on='Year', how='left')
    men_vs_women.rename(columns={'Name_x': 'Male', 'Name_y': 'Female'}, inplace=True)
    men_vs_women.fillna(0, inplace=True)
    return men_vs_women